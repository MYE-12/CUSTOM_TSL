...
# 		if self.get("start_date") and flt(payment_days) > flt(lwp):
# 			start_date = datetime.strptime(str(self.get("start_date")), '%Y-%m-%d')
# 			leave_salary = frappe.db.sql(
# 				"""
# 				SELECT name, leave_end_date
# 				FROM `tabLeave Salary`
# 				WHERE docstatus = 1
# 				AND employee = %s
# 				AND (
# 					(MONTH(from_date) = %s AND YEAR(from_date) = %s)
# 					OR (MONTH(leave_end_date) = %s AND YEAR(leave_end_date) = %s)
# 				)
...
# 			if leave_salary:
# 				holidays_ls = check_for_holidays_between_this_range(self.employee,start_date,leave_salary[0]['leave_end_date'])
# 				frappe.errprint(payment_days)
# 				frappe.errprint(lwp)
# 				frappe.errprint(count_)
# 				frappe.errprint(annual_count)
# 				frappe.errprint(holidays_ls)
# 				pay_days = flt(payment_days) - flt(lwp) - flt(count_) - flt(annual_count) - flt(holidays_ls) 
# 				if pay_days > 0:
...
# 	# def get_holidays_for_employee(self, start_date, end_date, skip_weekly_offs = False):
...
# # 	leave_end_date = "2025-06-17"
# # 	leave_salary = frappe.db.sql(
# # 		"""
# # 		SELECT name, from_date, to_date , leave_start_date ,leave_end_date
# # 		FROM `tabLeave Salary`
# # 		WHERE docstatus = 1
# # 		AND employee = %s
# # 		AND (
...
# # 	)[0]
# # 	print(leave_salary)
# # 	print(check_for_holidays_between_this_range(150,leave_start_date,leave_end_date))


# def check_for_holidays_between_this_range(employee,leave_start_date,leave_end_date):
...
# 		(leave_start_date, leave_end_date, holiday_list),
# 	)[0][0]
...
class CustomSalarySlip(TransactionBase):
	...
	def get_working_days_details(self, lwp=None, for_preview=0):
		...
		def get_overlap_days(employee, company, range_start, range_end, check_start, check_end):
			...
			# return (overlap_end - overlap_start).days + 1 if overlap_start <= overlap_end else 0
