...
def get_mc(name):
	...
	data+= '<tr>'
	data+= '<td style="text-align:center;background-color:#E0E0E0;width:10%"><b>WO</b></td>'
...
def purchase_report(date,company):
	...
	for i in wo_list_q:
		...
		and `tabWork Order Data`.name = '%s' ORDER BY `tabStatus Duration Details`.date desc LIMIT 1""" %(i.ct),as_dict=1)
	...
	for i in so_list_q:
		...
		and `tabSupply Order Data`.name = '%s' ORDER BY `tabStatus Duration Details`.date desc LIMIT 1""" %(i.ct),as_dict=1)
...
def get_lab(company):
    ...
    <style>
        ...
        /* Report Header */
        .report-header {
            background:linear-gradient(#135, #1565c0, #0d47a1);
            color: Black;
            padding: 35px;
            text-align: center;
            font-size: 4.2em;
    ...
    wo_count_weekly = frappe.db.count("Work Order Data",{"company":company,"posting_date": ["between", [s_week,e_week]]})

    data += '<td colspan = 1 style="border-color:#000000;color:#055c9d;text-align:center">%s</td>' %(wo_count_weekly)
    
    wo_count_monthly = frappe.db.count("Work Order Data",{"company":company,"posting_date": ["between", [s_month,e_month]]})

    data += '<td colspan = 1 style="border-color:#000000;color:#055c9d;text-align:center">%s</td>' %(wo_count_monthly) 
    
    wo_count_yearly = frappe.db.count("Work Order Data",{"company":company,"posting_date": ["between", [s_year,e_year]]})
    ...
    ner_wo_count_weekly = frappe.db.count("Work Order Data",{"company":company,"posting_date": ["between", [s_week,e_week]],"status":"NER-Need Evaluation Return"})

    data += '<td colspan = 1 style="border-color:#000000;color:#055c9d;text-align:center">%s</td>'%(ner_wo_count_weekly) 
    
    ner_wo_count_monthly = frappe.db.count("Work Order Data",{"company":company,"posting_date": ["between", [s_month,e_month]],"status":"NER-Need Evaluation Return"})
    ...
    and `tabWork Order Data`.posting_date between '%s' and '%s' """ %(company,s_week,e_week) ,as_dict=1)
    ...
    left join `tabStatus Duration Details` on `tabWork Order Data`.name = `tabStatus Duration Details`.parent
    where  `tabStatus Duration Details`.status = "RS-Repaired and Shipped" and `tabWork Order Data`.company = '%s'
    and `tabWork Order Data`.posting_date between '%s' and '%s' """ %(company,s_month,e_month) ,as_dict=1)
    ...
    left join `tabStatus Duration Details` on `tabWork Order Data`.name = `tabStatus Duration Details`.parent
    where  `tabStatus Duration Details`.status = "RS-Repaired and Shipped" and `tabWork Order Data`.company = '%s'
    and `tabWork Order Data`.posting_date between '%s' and '%s' and `tabWork Order Data`.status_cap = "NER-Need Evaluation Return" """ %(company,s_week,e_week) ,as_dict=1)
    ...
    left join `tabStatus Duration Details` on `tabWork Order Data`.name = `tabStatus Duration Details`.parent
    where  `tabStatus Duration Details`.status = "RS-Repaired and Shipped" and `tabWork Order Data`.company = '%s'
    and `tabWork Order Data`.posting_date between '%s' and '%s' and `tabWork Order Data`.status_cap = "NER-Need Evaluation Return" """ %(company,s_month,e_month) ,as_dict=1)
    ...
    # tr_count_weekly = frappe.db.count("Work Order Data",{"status":"TR-Technician Repair","company":company,"posting_date": ["between", [s_week,e_week]]})
    
    tr_count_weekly  = frappe.db.sql(""" select count(DISTINCT `tabWork Order Data`.name) as wd from `tabWork Order Data` 
    left join `tabStatus Duration Details` on `tabWork Order Data`.name = `tabStatus Duration Details`.parent
    where  `tabStatus Duration Details`.status = "TR-Technician Repair" and `tabWork Order Data`.company = '%s'
    and date(`tabStatus Duration Details`.date) between '%s' and '%s' """ %(company,s_week,e_week) ,as_dict=1)
    ...
    left join `tabStatus Duration Details` on `tabWork Order Data`.name = `tabStatus Duration Details`.parent
    where  `tabStatus Duration Details`.status = "TR-Technician Repair" and `tabWork Order Data`.company = '%s'
    and date(`tabStatus Duration Details`.date) between '%s' and '%s' """ %(company,s_month,e_month) ,as_dict=1)
    ...
    left join `tabStatus Duration Details` on `tabWork Order Data`.name = `tabStatus Duration Details`.parent
    where  `tabStatus Duration Details`.status = "TR-Technician Repair" and `tabWork Order Data`.company = '%s'
    and date(`tabStatus Duration Details`.date) between '%s' and '%s' """ %(company,s_year,e_year) ,as_dict=1)
    ...
    # not_repaired_count_weekly = frappe.db.count("Work Order Data",{"status": ["in", ["RNR-Return Not Repaired","RNF-Return No Fault"]],"company":company,"posting_date": ["between", [s_week,e_week]]})
    ...
    wo_list_time = frappe.get_all("Evaluation Report",{"company":company,"date": ["between", [s_week,e_week]]},["work_order_data"])
    total_hrs_ne = 0
    ...
    ne_time_ratio = (total_hrs_ne/ wo_count_weekly)/24

    data += '<td colspan = 1 style="border-color:#000000;color:#055c9d;text-align:center">%s</td>' %(round(ne_time_ratio,2))

    wo_list_time_month = frappe.get_all("Evaluation Report",{"company":company,"date": ["between", [s_month,e_month]]},["work_order_data"])
    total_hrs_ne_m = 0
    ...
    ne_time_ratio_m = (total_hrs_ne_m/ wo_count_monthly)/24
    data += '<td colspan = 1 style="border-color:#000000;color:#055c9d;text-align:center">%s</td>' %(round(ne_time_ratio_m,2))

    wo_list_time_yr = frappe.get_all("Evaluation Report",{"company":company,"date": ["between", [s_month,e_month]]},["work_order_data"])
    total_hrs_ne_y = 0
    ...
    ne_time_ratio_yr = (total_hrs_ne_y/ wo_count_yearly)/24
    ...
    and `tabQuotation`.transaction_date between '%s' and '%s' and `tabQuotation`.company = '%s' and  `tabQuotation`.pre_evaluation = 1 ''' %(s_week,e_week,company),as_dict=1)
    ...
    where `tabQuotation`.Workflow_state in ("Approved By Management") and
    `tabQuotation`.quotation_type in ("Internal Quotation - Repair") 
    and `tabQuotation`.transaction_date between '%s' and '%s' and `tabQuotation`.company = '%s' and  `tabQuotation`.pre_evaluation = 1 ''' %(s_month,e_month,company),as_dict=1)
    ...
    where `tabQuotation`.Workflow_state in ("Approved By Management") and
    `tabQuotation`.quotation_type in ("Internal Quotation - Repair") 
    and `tabQuotation`.transaction_date between '%s' and '%s' and `tabQuotation`.company = '%s' and  `tabQuotation`.pre_evaluation = 1 ''' %(s_year,e_year,company),as_dict=1)
    ...
    for i in technicians:
        ...
        wo_count_w_tech = frappe.db.count("Work Order Data",{"technician":i["tec"],"company":company,"posting_date": ["between", [s_week,e_week]]})

        data += '<td colspan = 1 style="border-color:#000000;color:#055c9d;text-align:center">%s</td>' %(wo_count_w_tech)

        wo_count_m_tech = frappe.db.count("Work Order Data",{"technician":i["tec"],"company":company,"posting_date": ["between", [s_month,e_month]]})

        data += '<td colspan = 1 style="border-color:#000000;color:#055c9d;text-align:center">%s</td>' %(wo_count_m_tech)

        wo_count_y_tech = frappe.db.count("Work Order Data",{"technician":i["tec"],"company":company,"posting_date": ["between", [s_year,s_year]]})
        ...
        left join `tabStatus Duration Details` on `tabWork Order Data`.name = `tabStatus Duration Details`.parent
        where  `tabStatus Duration Details`.status = "NER-Need Evaluation Return" and `tabWork Order Data`.company = '%s'
        and date(`tabStatus Duration Details`.date) between '%s' and '%s' and `tabWork Order Data`.technician = '%s' """ %(company,s_week,e_week,i["tec"]) ,as_dict=1)

        ner_tech_m = frappe.db.sql(""" select count(DISTINCT `tabWork Order Data`.name) as ct from `tabWork Order Data` 
        left join `tabStatus Duration Details` on `tabWork Order Data`.name = `tabStatus Duration Details`.parent
        where  `tabStatus Duration Details`.status = "NER-Need Evaluation Return" and `tabWork Order Data`.company = '%s'
        and date(`tabStatus Duration Details`.date) between '%s' and '%s' and `tabWork Order Data`.technician = '%s' """ %(company,s_month,e_month,i["tec"]) ,as_dict=1)

        ner_tech_y = frappe.db.sql(""" select count(DISTINCT `tabWork Order Data`.name) as ct from `tabWork Order Data` 
        left join `tabStatus Duration Details` on `tabWork Order Data`.name = `tabStatus Duration Details`.parent
        where  `tabStatus Duration Details`.status = "NER-Need Evaluation Return" and `tabWork Order Data`.company = '%s'
        and date(`tabStatus Duration Details`.date) between '%s' and '%s' and `tabWork Order Data`.technician = '%s' """ %(company,s_year,e_year,i["tec"]) ,as_dict=1)
        ...
        and wod.technician = '%s' and wod.status_cap = "NER-Need Evaluation Return" ''' %(s_week,e_week,company,i["tec"]),as_dict=1)
        ...
        and `tabQuotation`.transaction_date between '%s' and '%s' 
        and `tabQuotation`.company = '%s' 
        and wod.technician = '%s' and wod.status_cap = "NER-Need Evaluation Return" ''' %(s_month,e_month,company,i["tec"]),as_dict=1)
        ...
        and wod.technician = '%s' and wod.status_cap = "NER-Need Evaluation Return" ''' %(s_year,e_year,company,i["tec"]),as_dict=1)
        ...
        left join `tabStatus Duration Details` on `tabWork Order Data`.name = `tabStatus Duration Details`.parent
        where `tabStatus Duration Details`.status IN ("RNF-Return No Fault","RNR-Return Not Repaired") and `tabWork Order Data`.company = '%s'
        and `tabWork Order Data`.posting_date between '%s' and '%s' and `tabWork Order Data`.technician = '%s' """ %(company,s_week,e_week,i["tec"]) ,as_dict=1)


        not_repaired_count_monthy_t  = frappe.db.sql(""" select count(DISTINCT `tabWork Order Data`.name) as ct from `tabWork Order Data` 
        left join `tabStatus Duration Details` on `tabWork Order Data`.name = `tabStatus Duration Details`.parent
        where `tabStatus Duration Details`.status IN ("RNF-Return No Fault","RNR-Return Not Repaired") and `tabWork Order Data`.company = '%s'
        and `tabWork Order Data`.posting_date between '%s' and '%s' and `tabWork Order Data`.technician = '%s' """ %(company,s_month,e_month,i["tec"]) ,as_dict=1)


        not_repaired_count_yearly_t  = frappe.db.sql(""" select count(DISTINCT `tabWork Order Data`.name) as ct from `tabWork Order Data` 
        left join `tabStatus Duration Details` on `tabWork Order Data`.name = `tabStatus Duration Details`.parent
        where `tabStatus Duration Details`.status IN ("RNF-Return No Fault","RNR-Return Not Repaired") and `tabWork Order Data`.company = '%s'
        and `tabWork Order Data`.posting_date between '%s' and '%s' and `tabWork Order Data`.technician = '%s' """ %(company,s_year,e_year,i["tec"]) ,as_dict=1)
        ...
        where `tabQuotation`.Workflow_state in ("Approved By Customer","Quoted to Customer","Rejected by Customer") and
        `tabQuotation`.quotation_type in ("Customer Quotation - Repair","Revised Quotation - Repair") 
        and `tabQuotation`.transaction_date between '%s' and '%s' and `tabQuotation`.company = '%s' and wod.technician = '%s' ''' %(s_week,e_week,company,i["tec"]),as_dict=1)
        ...
        where `tabQuotation`.Workflow_state in ("Approved By Customer","Quoted to Customer","Rejected by Customer") and
        `tabQuotation`.quotation_type in ("Customer Quotation - Repair","Revised Quotation - Repair") 
        and `tabQuotation`.transaction_date between '%s' and '%s' and `tabQuotation`.company = '%s' and wod.technician = '%s' ''' %(s_month,e_month,company,i["tec"]),as_dict=1)
        ...
        where `tabQuotation`.Workflow_state in ("Approved By Customer","Quoted to Customer","Rejected by Customer") and
        `tabQuotation`.quotation_type in ("Customer Quotation - Repair","Revised Quotation - Repair") 
        and `tabQuotation`.transaction_date between '%s' and '%s' and `tabQuotation`.company = '%s' and wod.technician = '%s' ''' %(s_year,e_year,company,i["tec"]),as_dict=1)
