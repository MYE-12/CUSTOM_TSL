...
def create_supply_order_data(order_no):
	...
	new_doc.tender_closing = doc.tender_closing
	new_doc.supplier_closing_date = doc.supplier_closing_date
	...
	# if frappe.session.user == "supply-kw2@tsl-me.com" or "supply1@tsl-me.com":
	# 	new_doc.st = 1
...
@frappe.whitelist()
def create_supply_order_data_uae(order_no):
	l=[]
	d = {
			"Dammam - TSL-SA":"SOD-D.YY.-",
			"Riyadh - TSL- KSA":"SOD-R.YY.-",
...
@frappe.whitelist()
def create_supply_tender_kuwait(order_no):
	l=[]
	d = {
			"Dammam - TSL-SA":"SOD-D.YY.-",
			"Riyadh - TSL- KSA":"SOD-R.YY.-",
	...
	if doc.company == "TSL COMPANY - Kuwait" and doc.department == "Supply Tender - TSL":
		new_doc.naming_series ="ST-K.YY.-"
	
	if doc.company == "TSL COMPANY - Kuwait" and doc.department == "Supply - TSL":
		new_doc.naming_series ="SOD-DU.YY.-"
...
@frappe.whitelist()
def create_supply_order_data_kuwait(order_no):
	l=[]
	d = {
			"Dammam - TSL-SA":"SOD-D.YY.-",
			"Riyadh - TSL- KSA":"SOD-R.YY.-",
	...
	l.append(new_doc.name)
	if l:
		frappe.delete_doc("Create Supply Order Kuwait","Create Supply Order Kuwait")
		link = []
		for i in l:
			link.append(""" <a href='/app/supply-order-data/{0}'>{0}</a> """.format(i))
		frappe.msgprint("Supply Order created: "+', '.join(link))
...
@frappe.whitelist()
def create_supply_order_data_ksa(order_no):
	l=[]
	d = {
			"Dammam - TSL-SA":"SOD-D.YY.-",
			"Riyadh - TSL- KSA":"SOD-R.YY.-",
	...
	if l:
		if doc.branch == "Jeddah - TSL-SA":
			frappe.delete_doc("Create Supply Order Jeddah", "Create Supply Order Jeddah")
		if doc.branch == " Riyadh - TSL- KSA":
			frappe.delete_doc("Create Supply Order Riyadh", "Create Supply Order Riyadh")
		if doc.branch == "Dammam - TSL-SA":
			frappe.delete_doc("Create Supply Order Dammam", "Create Supply Order Dammam")
