	...
	def get_work_orders(self):
		...
		if self.report == "RSI-Repaired and Shipped Invoiced":
			...
			if self.customer:
				...
				where `tabSales Invoice`.due_date between '%s' and '%s' and `tabSales Invoice`.customer = '%s' ORDER BY `tabSales Invoice`.due_date ASC """ %(self.from_date,self.to_date,self.customer),as_dict =1)
				...
				where `tabSales Team`.sales_person = '%s' and `tabSales Invoice`.due_date between '%s' and '%s' ORDER BY `tabSales Invoice`.due_date ASC """ %(self.sales_person,self.from_date,self.to_date),as_dict =1)

			elif self.work_order_data:
				si = frappe.db.sql(""" select DISTINCT `tabSales Invoice`.due_date,`tabSales Invoice Item`.work_order_data as wo,`tabSales Invoice Item`.wod_no as w from `tabSales Invoice` 
				left join `tabSales Invoice Item` on `tabSales Invoice`.name = `tabSales Invoice Item`.parent 
				where `tabSales Invoice`.due_date between '%s' and '%s' and `tabSales Invoice Item`.work_order_data = '%s' ORDER BY `tabSales Invoice`.due_date ASC """ %(self.from_date,self.to_date,self.work_order_data),as_dict =1)
				if not si:
					si = frappe.db.sql(""" select DISTINCT `tabSales Invoice`.due_date,`tabSales Invoice Item`.work_order_data as wo,`tabSales Invoice Item`.wod_no as w from `tabSales Invoice` 
					left join `tabSales Invoice Item` on `tabSales Invoice`.name = `tabSales Invoice Item`.parent 
					where `tabSales Invoice`.due_date between '%s' and '%s' and `tabSales Invoice Item`.wod_no = '%s' ORDER BY `tabSales Invoice`.due_date ASC """ %(self.from_date,self.to_date,self.work_order_data),as_dict =1)

			else:
				si = frappe.db.sql(""" select DISTINCT `tabSales Invoice`.due_date,`tabSales Invoice Item`.work_order_data as wo,`tabSales Invoice Item`.wod_no as w from `tabSales Invoice` 
				left join `tabSales Invoice Item` on `tabSales Invoice`.name = `tabSales Invoice Item`.parent 
				where `tabSales Invoice`.due_date between '%s' and '%s' ORDER BY `tabSales Invoice`.due_date ASC """ %(self.from_date,self.to_date),as_dict =1)
		...
		if self.report == "RSC-Repaired and Shipped Client":
			...
			data += '<td style="border-color:#000000;width:15%;padding:1px;font-size:14px;font-size:12px;background-color:#3333ff;color:white;"><center><b>Delivery Note</b><center></td>'
