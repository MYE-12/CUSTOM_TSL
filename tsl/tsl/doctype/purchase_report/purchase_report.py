...
class PurchaseReport(Document):
	...
	def get_data(self):
		...
		data += '<td colspan = 2 style="border-color:#000000;"><h2><center><b>TSL Company</b></center></h2></td>'
		...
		and `tabStatus Duration Details`.date  LIKE "%s%%" """ %(self.date) ,as_dict=1)
		...
		not_qt =  frappe.db.count("Work Order Data",{"company":"TSL COMPANY - Kuwait","old_wo_no":["is","not set"],"status":"SP-Searching Parts","posting_date": ["between", ["2015-07-23", self.date]]})

		wp =  frappe.db.count("Work Order Data",{"status":"WP-Waiting Parts","posting_date": ["between", ["2015-07-23", self.date]]})
		...
		s_not_quoted = frappe.db.count("Supply Order Data",{"department":"Supply - TSL","docstatus":1,"company":"TSL COMPANY - Kuwait","status":"Inquiry","posting_date": ["between", ["2015-07-23", self.date]]})

		s_not_found = frappe.db.count("Supply Order Data",{"status":"Not Found","posting_date": ["between", ["2015-07-23", self.date]]})
		...
		prw = frappe.db.sql(""" select count(`tabPurchase Receipt Item`.work_order_data) as pr from `tabPurchase Receipt`
			...
			where `tabPurchase Receipt`.posting_date = '%s' and `tabPurchase Receipt`.docstatus != 2  and `tabPurchase Receipt`.company = "TSL COMPANY - Kuwait" """ %(self.date),as_dict =1)
		...
		pre_eval =  frappe.get_all("Work Order Data",{"status":"A-Approved","company":"TSL COMPANY - Kuwait","old_wo_no":["is","not set"],"posting_date": ["between", ["2015-07-23", self.date]]})
		...
		sq = frappe.db.count("Supplier Quotation",{"transaction_date":self.date,"work_order_data": ["is", "set"],"company":"TSL COMPANY - Kuwait"})
		...
		spo = frappe.db.sql(""" select count(`tabPurchase Order Item`.supply_order_data) as p from `tabPurchase Order`
			left join `tabPurchase Order Item` on `tabPurchase Order Item`.parent = `tabPurchase Order`.name
			where `tabPurchase Order`.transaction_date = '%s' and `tabPurchase Order`.docstatus != 2 and `tabPurchase Order`.per_received = %s and `tabPurchase Order`.company = "TSL COMPANY - Kuwait" """ %(self.date,0),as_dict =1)
