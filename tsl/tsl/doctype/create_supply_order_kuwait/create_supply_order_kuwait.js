...
frappe.ui.form.on('Create Supply Order Kuwait', {
	refresh:function(frm){
		frm.disable_save()
		frm.add_custom_button(__("Create Supply Order"), function(){
			frappe.call({
				'method': 'tsl.tsl.doctype.supply_order_form.supply_order_form.create_supply_order_data_kuwait',
				'freeze':true,
				'args':{
	...
	branch:function(frm){
		if(frm.doc.company && frm.doc.branch){
			...
			// frm.set_value("department",d[frm.doc.branch]);
