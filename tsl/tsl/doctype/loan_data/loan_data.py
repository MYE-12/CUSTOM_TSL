...
class LoanData(Document):
    ...
    def set_missing_fields(self):
        ...
        if self.loan_product and not self.rate_of_interest:
            self.rate_of_interest = frappe.db.get_value(
                "Loan Type", self.loan_product, "rate_of_interest"
            )
    ...
    # def request_loan_closure(loan, posting_date=None):
        ...
        # write_off_limit = frappe.get_value("Loan Type", loan_product, "write_off_amount")
    ...
    def make_repayment_schedule(self):
        ...
        schedule_type_details = frappe.db.get_value(
            "Loan Type", self.loan_product, ["repayment_schedule_type", "repayment_date_on"], as_dict=1
        )
...
@frappe.whitelist()
def create_additional_salary(name,is_deducted,company,employee,payment_date,total_payment):
    if is_deducted == "0":
        exist = frappe.db.exists("Additional Salary",{'employee':employee,'payroll_date':payment_date})
        if exist:
            frappe.msgprint("Additional Salary already there - <b>"+exist+"</b>")
        else:
            doc = frappe.new_doc("Additional Salary")
            doc.employee = employee
            doc.company = company
            doc.salary_component = "Loan"
            doc.overwrite_salary_structure_amount = 1
            doc.payroll_date = payment_date
            doc.amount = int(total_payment)
            doc.save(ignore_permissions=True)
            # doc.submit()ss
            frappe.msgprint("Additional Salary Created - <b>"+doc.name+"</b>")
            frappe.db.set_value("Repayment Schedule",name,'is_deducted',1)
    else:
        exist = frappe.db.exists("Additional Salary",{'employee':employee,'payroll_date':payment_date})
        if exist:
            frappe.msgprint("Additional Salary already there - <b>"+exist+"</b>")
...
def update_schedule():
    sch_date = today()
    ld = frappe.db.sql(""" select name from `tabLoan Data` where docstatus = 1 """,as_dict=1)
    for l in ld:
        doc = frappe.get_doc("Loan Data", l.name)        
        for i, row in enumerate(doc.repayment_schedule):
            if str(row.payment_date) == sch_date:
                exist = frappe.db.exists("Additional Salary", {'employee': doc.applicant, 'payroll_date': row.payment_date})
                if not exist:
                    new_payment_date = get_next_month_date(row.payment_date)
                    print(new_payment_date)
                    frappe.db.set_value("Repayment Schedule", row.name, "payment_date", new_payment_date)
                    
                    previous_date = new_payment_date
                    for j in range(i + 1, len(doc.repayment_schedule)):
                        next_payment_date = get_next_month_date(previous_date)
                        frappe.db.set_value("Repayment Schedule", doc.repayment_schedule[j].name, "payment_date", next_payment_date)
                        previous_date = next_payment_date
                    
                    frappe.msgprint("Payment schedule updated successfully.")
                    break
        
        frappe.msgprint("No updates needed.")

def get_next_month_date(date):
    next_month = date.replace(day=1) + timedelta(days=calendar.monthrange(date.year, date.month)[1])
    return next_month
