...
class ProjectData(Document):
		...
		for i in self.items:
			if i.order_type == "Repair":
				for j in range(i.qty):
					...
					s.automation = i.automation
					s.priority_status = "Normal"
					if i.automation == 1:	
						s.naming_series = "WOD-DU.YY.-PA."
						s.department = "Automation - TSL"
					else:	
						s.naming_series = "WOD-DU.YY.-P."
					s.project = self.name
	...
	def crt_quote(self):
		...
		new_doc.naming_series = "PIQ-DU.YY.-"
	...
	def rfq(self):
		...
		new_doc.project_data = self.name
		# new_doc.quotation_type = "Internal Quotation - Project"
		for i in self.items:
